<div
  ...
  <div class="flex items-start w-0 flex-1 space-x-3">
    ...
    <label class="relative cursor-pointer flex items-center group flex-shrink-0 mt-0.5">
      ...
      <div
        class="w-6 h-6 rounded-full border-2 border-gray-300 bg-white flex items-center justify-center transition-all duration-300 shadow-sm group-hover:shadow-md peer-checked:bg-blue-100">
        <svg
          class="w-4 h-4 text-blue-500 opacity-0 group-hover:opacity-100 transition duration-200 peer-checked:opacity-100"
    ...
    <div class="flex-1 overflow-hidden">
      <div x-data="{
        ...
        }" x-on:keydown.escape.prevent="cancel()" class="w-full max-w-full overflow-hidden">
        {{-- ===== TITLE ===== --}}
        @if ($editingTaskId === $task->id)
          <textarea wire:model.defer="editingTitle"
            ...
            style="padding:0;min-height:0;height:auto;line-height:1.2;" autofocus></textarea>
        @else
          <div @click="start()"
        ...
        @if ($editingTaskId === $task->id)
          <textarea wire:model.defer="editingDescription"
            ...
            style="padding:2px 0;min-height:50px;overflow-wrap:break-word;white-space:pre-wrap;"></textarea>
        @else
          <div @click="start()"
        ...
        {{-- ===== DUE DATE ===== --}}
        @if ($editingTaskId === $task->id)
          <input type="date" wire:model.defer="editingDueDate"
        ...
        @else
          ...
          @else
            <div @click="start()" class="mt-1 text-sm text-gray-400 italic cursor-text hover:opacity-70">
        ...
        <div x-data="{
          ...
          }" x-init="$watch('q', () => updateSug())" class="flex flex-wrap gap-1 mt-2 relative">
          {{-- chips --}}
  ...
  <div class="flex items-center space-x-2 transition"
    ...
    @else
      {{-- Star --}}
      <button type="button" wire:click="toggleStar({{ $task->id }})" class="text-yellow-500 hover:text-yellow-600"
        title="Beri bintang">
        <svg class="w-5 h-5" fill="{{ $task->is_starred ? 'currentColor' : 'none' }}" stroke="currentColor"
          viewBox="0 0 24 24">
      ...
      {{-- Edit --}}
      <button type="button" wire:click="startEditing({{ $task->id }})" class="text-blue-500 hover:text-blue-700"
        title="Edit">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
