<div wire:poll class="min-h-screen bg-gray-50">
    ...
    <div class="flex-1 p-6">
      <div class="flex justify-between items-center mb-5">
        ...
        <div class="flex items-center space-x-4">
          ...
          <a href="{{ route('calendar.admin') }}" wire:navigate
          ...
          <button wire:click="openCreateModal" wire:loading.attr="disabled" wire:target="openCreateModal,createTask"
      ...
      <div x-data="{
        ...
        }" class="fixed top-6 right-6 z-50" aria-live="polite">
          ...
          <div class="flex gap-3 p-3.5">
            <div class="mt-0.5 shrink-0">
              <template x-if="type==='success'">
                <svg class="h-5 w-5" :style="`color:${palette[type].icon}`" fill="none" viewBox="0 0 24 24"
                  stroke="currentColor">
      ...
      @if ($showCreateModal)
        <div x-data="{}" x-cloak x-transition.opacity class="fixed inset-0 z-[9998] flex items-center justify-center"
            ...
            <div class="px-5 py-4 max-h-[75vh] overflow-y-auto">
              <form wire:submit.prevent="createTask" class="space-y-4">
                ...
                <div>
                  ...
                  <input type="date" wire:model.defer="newTask.due_date"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
              ...
              }" x-init="$watch('query', () => updateSuggestion())" class="mt-2 relative">
                  <label class="block text-sm font-medium text-gray-700 mb-1">Partisipan</label>

                  {{-- Input + ghost suggestion --}}
                  <div class="relative">
                    <input type="text" x-model="query" @keydown.tab.prevent="acceptGhost()"
                      @keydown.enter.prevent="enter()" placeholder="Ketik nama partisipan…"
                    ...
                    <div class="absolute z-10 mt-1 w-full bg-white border rounded-lg shadow max-h-56 overflow-y-auto"
                      ...
                      <template x-for="p in sug" :key="p.id ?? p.name">
                        <button type="button" class="w-full text-left px-3 py-2 hover:bg-slate-50" @click="pick(p)"
  ...
  <div x-data="{
    ...
    }" x-show="open" x-cloak class="fixed inset-0 z-[9998] flex items-center justify-center">
